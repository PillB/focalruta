...
def _draw_tercios(ax):
    ...
    subj_x, subj_y = 0.5 + 11/3, 0.5 + 2*7/3
...
def _draw_perspectiva(ax):
    ...
    for i in range(1, 5):
        offset = i * 0.7
        ax.plot([1 + offset, 11 - offset], [1 + i*0.15, 1 + i*0.15],
                color=PALETTE["muted"], linewidth=0.5, alpha=0.4)


def _draw_simetria(ax):
...
def _draw_capas(ax):
    ...
    for i, (color, label) in enumerate(zip(layer_colors, layer_labels)):
        ...
        ax.text(0.5, y, label, fontsize=7, color=color, fontweight="bold", ha="right", va="center")
        if i == 0:
