...
function updatePerspective(){const position=Number(document.querySelector('#perspective-position').value),focal=Number(document.querySelector('#perspective-focal').value),scale=focal/35,near=70*scale/position,far=70*scale/(position+4),ratio=(near/far).toFixed(2);document.querySelector('#perspective-near').setAttribute('height',Math.min(100,near));document.querySelector('#perspective-near').setAttribute('y',112-Math.min(100,near));document.querySelector('#perspective-far').setAttribute('height',Math.min(100,far));document.querySelector('#perspective-far').setAttribute('y',112-Math.min(100,far));document.querySelector('#perspective-feedback').textContent=`Posición ${position} m · ${focal} mm. Relación de tamaño cercano/lejos: ${ratio}×. Cambiar focal escala ambos; mover la cámara cambia la relación.`}
function updateVertical(){const tilt=Number(document.querySelector('#vertical-tilt').value),inset=tilt*1.4;document.querySelector('#vertical-building').setAttribute('points',`${28+inset},18 ${192-inset},18 192,112 28,112`);document.querySelector('#vertical-feedback').textContent=tilt===0?'Cámara nivelada: verticales paralelas en este modelo.':'Inclinación '+tilt+'°: las verticales convergen. Decide si expresa altura o distrae del contrato espacial.'}
...
function updateLight(){const mode=document.querySelector('#light-mode').value,shadow=document.querySelector('#light-shadow'),face=document.querySelector('#light-face');const states={side:{shadow:'70,90 180,112 105,112',fill:'#c66b3d',opacity:'.8',copy:'Luz lateral: relieve y textura ganan separación por gradiente y sombra.'},noon:{shadow:'70,90 112,101 84,105',fill:'#e7a563',opacity:'.95',copy:'Mediodía: sombra corta y contraste gráfico; vigila huecos negros y altas luces.'},garua:{shadow:'70,90 95,96 80,99',fill:'#9ca9a2',opacity:'.2',copy:'Garúa: bordes de sombra suaves; color y microtextura pueden separar materiales.'},back:{shadow:'40,90 12,112 70,112',fill:'#364b46',opacity:'.95',copy:'Contraluz: la silueta domina; conserva detalle sólo si la relación lo necesita.'}}[mode];shadow.setAttribute('points',states.shadow);shadow.style.opacity=states.opacity;face.setAttribute('fill',states.fill);document.querySelector('#light-feedback').textContent=states.copy}
function updateComposition(){const mode=document.querySelector('#composition-mode').value,frame=document.querySelector('#composition-frame'),copy={"default-postcard":'Postal: eje central y edificio completo; legible, pero la relación urbana todavía es genérica.',"changed-position":'Posición: el desplazamiento lateral revela umbral, profundidad y solapes nuevos.',"fixed-position-focal":'Focal a posición fija: cambia el recorte, no la perspectiva entre los planos.',"human-presence":'Presencia: el gesto solo suma si demuestra cómo se usa el espacio; compara también la ausencia.',"light-weather":'Luz y clima: otra dirección o garúa puede separar material y volumen sin cambiar la geometría.'};frame.setAttribute('data-mode',mode);document.querySelector('#composition-feedback').textContent=copy[mode]}
...
<article class="learning-lab"><p class="eyebrow">LAB 5 · SECUENCIA</p><h3>{escape(composition["title"])}</h3><p>{escape(composition["prediction_prompt"])}</p><label for="composition-mode">Decisión aislada</label><select id="composition-mode">{''.join(f'<option value="{escape(item["variant_id"])}">{escape(item["label"])}</option>' for item in composition["variants"])}</select><svg id="composition-frame" class="lab-frame" role="img" aria-label="Una escena arquitectónica comparada mediante cinco decisiones" viewBox="0 0 220 120"><path d="M25 108V35H145V108M70 108V68H110V108" fill="#80958d" stroke="#10211d" stroke-width="4"/><path d="M145 55L202 82V108H145Z" fill="#c66b3d"/><circle cx="90" cy="78" r="7" fill="#f2cf45"/></svg><p id="composition-feedback" class="lab-readout" role="status"></p><p><strong>Campo:</strong> {escape(composition["field_drill"])}</p><p class="route-note">{escape(composition["model_limit"])}</p><p class="source-links">{source_links(composition["sources"])}</p></article>'''
...
<p><a href="index.html">Volver a Arquitectura en Foco</a></p></main></body></html>'''
