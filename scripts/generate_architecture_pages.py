...
PHYSICS_GUIDE = '<aside class="beginner-guide"><h3>Modelo físico honesto</h3><p>La relación <strong>focal/posición</strong> separa dos causas: la posición controla la perspectiva y la focal controla el encuadre desde una posición fija. En una superficie mate, el brillo difuso cae aproximadamente con <strong>cos(θ)</strong> (θ es el ángulo entre la luz y la perpendicular de la cara). Las sombras siguen la dirección de la luz: una fuente grande produce una penumbra suave y una fuente pequeña un borde duro. Los brillos, reflejos y halos dependen de material, ángulo y exposición; aquí se muestran como señales causales: no es una simulación calibrada ni una predicción de píxeles.</p></aside>'
...
function updatePerspective(){const position=Number(document.querySelector('#perspective-position').value),focal=Number(document.querySelector('#perspective-focal').value),scale=focal/35,near=70*scale/position,far=70*scale/(position+4),ratio=(near/far).toFixed(2),halfFov=Math.min(58,22+focal/8),nearWidth=Math.min(70,near*.64),farWidth=Math.min(70,far*.64);const nearRect=document.querySelector('#perspective-near'),farRect=document.querySelector('#perspective-far');nearRect.setAttribute('height',Math.min(100,near));nearRect.setAttribute('y',112-Math.min(100,near));nearRect.setAttribute('width',nearWidth);nearRect.setAttribute('x',80-nearWidth/2);farRect.setAttribute('height',Math.min(100,far));farRect.setAttribute('y',112-Math.min(100,far));farRect.setAttribute('width',farWidth);farRect.setAttribute('x',157-farWidth/2);document.querySelector('#perspective-fov').setAttribute('d',`M8 112L52 ${112-halfFov}M8 112L52 ${112+halfFov}`);document.querySelector('#perspective-feedback').textContent=`Posición ${position} m · ${focal} mm. Proyección h≈f·H/z; relación cercano/lejos ${ratio}×. La focal abre o cierra el campo de visión; la distancia cambia la relación.`}
...
function updateLight(){const mode=document.querySelector('#light-mode').value,shadow=document.querySelector('#light-shadow'),umbra=document.querySelector('#light-umbra'),face=document.querySelector('#light-face'),direction=document.querySelector('#light-direction'),blur=document.querySelector('#light-penumbra');const states={side:{shadow:'70,90 180,112 105,112',umbra:'78,92 150,108 108,108',fill:'url(#light-gradient)',opacity:'.8',blur:'1.2',x1:18,y1:24,x2:70,y2:48,copy:'Luz lateral: relieve y textura ganan separación por gradiente y sombra.'},noon:{shadow:'70,90 112,101 84,105',umbra:'76,91 120,101 88,104',fill:'#e7a563',opacity:'.95',blur:'.4',x1:28,y1:10,x2:80,y2:34,copy:'Mediodía: sombra corta y dura; vigila huecos negros y altas luces.'},garua:{shadow:'70,90 95,96 80,99',umbra:'82,92 98,96 86,98',fill:'#9ca9a2',opacity:'.2',blur:'4',x1:8,y1:38,x2:70,y2:54,copy:'Garúa: fuente amplia, penumbra suave; color y microtextura separan materiales.'},back:{shadow:'40,90 12,112 70,112',umbra:'52,91 28,108 64,108',fill:'#364b46',opacity:'.95',blur:'1',x1:210,y1:20,x2:145,y2:48,copy:'Contraluz: la silueta domina; conserva detalle sólo si la relación lo necesita.'}}[mode];shadow.setAttribute('points',states.shadow);umbra.setAttribute('points',states.umbra);shadow.style.opacity=states.opacity;face.setAttribute('fill',states.fill);direction.setAttribute('x1',states.x1);direction.setAttribute('y1',states.y1);direction.setAttribute('x2',states.x2);direction.setAttribute('y2',states.y2);blur.querySelector('feGaussianBlur').setAttribute('stdDeviation',states.blur);document.querySelector('#light-feedback').textContent=states.copy}
...
<article class="learning-lab"><p class="eyebrow">LAB 5 · SECUENCIA</p><h3>{escape(composition["title"])}</h3><p>{escape(composition["prediction_prompt"])}</p><label for="composition-mode">Decisión aislada</label><select id="composition-mode">{''.join(f'<option value="{escape(item["variant_id"])}">{escape(item["label"])}</option>' for item in composition["variants"])}</select><svg id="composition-frame" class="lab-frame" role="img" aria-label="Comparación antes y después de cinco decisiones" viewBox="0 0 440 180"><rect class="comparison-pane" x="5" y="5" width="205" height="170"/><rect class="comparison-pane" x="230" y="5" width="205" height="170"/><text x="18" y="25">ANTES</text><text x="243" y="25">DESPUÉS</text><g id="composition-before"><path d="M35 155V55H180V155M85 155V105H130V155" fill="#80958d" stroke="#10211d" stroke-width="4"/><circle cx="108" cy="116" r="8" fill="#f2cf45"/></g><g id="composition-after"><g class="state-layer" data-state="default-postcard"><path d="M260 155V55H405V155M310 155V105H355V155" fill="#80958d" stroke="#10211d" stroke-width="4"/><circle cx="333" cy="116" r="8" fill="#f2cf45"/></g><g class="state-layer" data-state="changed-position"><path d="M245 155L278 48H408V155M325 155V98H370V155" fill="#80958d" stroke="#10211d" stroke-width="4"/><path d="M278 48L245 155" stroke="#b84c32" stroke-width="6"/></g><g class="state-layer" data-state="fixed-position-focal"><path d="M235 165V35H430V165M300 165V88H365V165" fill="#80958d" stroke="#10211d" stroke-width="5"/><rect x="260" y="46" width="145" height="105" fill="none" stroke="#b84c32" stroke-width="3" stroke-dasharray="7 5"/></g><g class="state-layer" data-state="human-presence"><path d="M260 155V55H405V155M310 155V105H355V155" fill="#80958d" stroke="#10211d" stroke-width="4"/><circle cx="333" cy="105" r="10" fill="#b84c32"/><path d="M333 115v32m0-20l-18 12m18-12l18 12" stroke="#b84c32" stroke-width="6"/></g><g class="state-layer" data-state="light-weather"><path d="M260 155V55H405V155M310 155V105H355V155" fill="#7891a0" stroke="#10211d" stroke-width="4"/><path d="M235 35l75 35M248 28l75 35M365 55l60 60" stroke="#5d7f91" stroke-width="4"/><polygon points="355,155 425,120 425,155" fill="#364b46"/></g></g><g id="composition-fixed-position" opacity="0"><path d="M205 145h30" stroke="#176b55" stroke-width="4"/><circle cx="220" cy="145" r="8" fill="#176b55"/><text x="170" y="168">cámara fija</text></g></svg><p id="composition-feedback" class="lab-readout" role="status"></p><p><strong>Campo:</strong> {escape(composition["field_drill"])}</p><p class="route-note">{escape(composition["model_limit"])}</p><p class="source-links">{source_links(composition["sources"])}</p></article>'''
    ...
    html = html.replace('<rect id="light-face"', '<polygon id="light-umbra" points="78,92 142,108 108,108" fill="#263833" opacity=".72"/><rect id="light-face"', 1)
    html = html.replace('<svg id="composition-frame"', '<svg id="composition-frame"', 1)
    html = html.replace('</svg><p id="composition-feedback"', '</svg><p class="viz-caption">Visualización: ANTES conserva la referencia; DESPUÉS muestra qué relación cambia. La marca “cámara fija” significa que solo se recortó con la focal.</p><p id="composition-feedback"', 1)
    html = html.replace('<g class="state-layer" data-state="light-weather"><path', '<g class="state-layer" data-state="light-weather"><defs><radialGradient id="composition-glare"><stop offset="0" stop-color="#fff7c2" stop-opacity=".95"/><stop offset="1" stop-color="#fff7c2" stop-opacity="0"/></radialGradient><filter id="composition-halo-filter"><feGaussianBlur stdDeviation="5"/></filter></defs><path', 1)
    html = html.replace('<polygon points="355,155 425,120 425,155" fill="#364b46"/>', '<polygon points="355,155 425,120 425,155" fill="#364b46"/><path id="composition-reflection" d="M280 62L382 150M296 55L398 143" stroke="#d8edf0" stroke-width="5" opacity=".55"/><circle id="composition-halo" cx="382" cy="58" r="25" fill="url(#composition-glare)" filter="url(#composition-halo-filter)"/><circle id="composition-glare" cx="382" cy="58" r="8" fill="#fff7c2"/>', 1)
