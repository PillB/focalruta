...
:root{{--ink:#14231f;--paper:#f7f3e9;--accent:#b84c32;--line:#c9c0ad}}*{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font:17px/1.55 system-ui,sans-serif}}main{{max-width:920px;margin:auto;padding:clamp(20px,5vw,56px)}}h1{{font-size:clamp(2rem,8vw,4.8rem);line-height:.95}}section{{border-top:1px solid var(--line);padding:28px 0}}.rule{{font-weight:750}}.warning{{border-left:5px solid var(--accent);padding:14px 18px;background:#fff}}label{{display:block;margin:12px 0 4px}}input,button{{font:inherit;min-height:44px}}input[type=file]{{max-width:100%}}button{{margin-top:16px;padding:8px 18px;background:var(--ink);color:white;border:0;border-radius:6px}}button:focus-visible,input:focus-visible{{outline:3px solid var(--accent);outline-offset:3px}}@media(prefers-reduced-motion:reduce){{*{{scroll-behavior:auto!important}}}}
