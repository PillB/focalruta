...
MATRIX=r'''()=>{const q=s=>document.querySelector(s),fire=(e,t='input')=>e.dispatchEvent(new Event(t,{bubbles:true})),lens=q('#opt-lens'),ap=q('#opt-ap'),dist=q('#opt-dist'),bg=q('#opt-bg'),light=q('#opt-light'),shutter=q('#opt-shutter'),iso=q('#opt-iso'),failures=[],floors=[];let optical=0,exposure=0;
 ...
 for(let ev=+light.min;ev<=+light.max;ev+=+light.step)for(const t of [...shutter.options].map(x=>x.value))for(const s of [...iso.options].map(x=>x.value)){light.value=ev;fire(light);shutter.value=t;fire(shutter,'change');iso.value=s;fire(iso,'change');exposure++;exposureStates.add(q('#opt-exposure-overlay').getAttribute('fill')+'|'+q('#opt-exposure-overlay').getAttribute('opacity')+'|'+q('#opt-light-label').textContent);if(geometry()!==base)failures.push([ev,t,s,'exposure changed geometry'])}
...
with sync_playwright() as pw:
    ...
    for target_name,target in targets.items():
        ...
        check(f'{target_name} every exposure combination',matrix['exposure']==2093,{'states':matrix['exposure'],'visualStates':matrix['exposureStates']})
