...
def map_html(layer: dict, filename: str) -> None:
    ...
    content = f'<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Mapa offline · {escape(layer["district"])}</title><style>body{{font:16px/1.5 system-ui;max-width:900px;margin:auto;padding:1rem;color:#10211d}}svg{{width:100%;background:#eef0e9;border:1px solid #cbc2b2}}polyline{{fill:none;stroke:#b84c32;stroke-width:7}}circle{{fill:#10211d;stroke:#fff;stroke-width:3}}text{{fill:#fff;font:bold 12px system-ui;text-anchor:middle;dominant-baseline:central}}li{{margin:.8rem 0}}</style><h1>{escape(layer["district"])} · mapa offline</h1><p>Geometría peatonal capturada; confirma cruces y cierres en campo. Google Maps puede recalcular otra ruta.</p><svg viewBox="0 0 800 400" role="img" aria-label="Ruta peatonal con paradas numeradas">{lines}{markers}</svg><p>Visualización: la línea muestra el trazado capturado y los números el orden de las paradas; no es navegación en vivo.</p><ol>{rows}</ol>'
